...
const handleSchoolLogoChange = (event) => {
  ...
  schoolStore.setSchoolLogoFile(file);
};

const deleteSchool = async (item) => {
  if (!window.confirm(`Hapus sekolah "${item.name}"? Semua data sekolah ini juga akan dihapus.`)) return;
  try {
    const response = await schoolStore.deleteSchool(item);
    if (response && !editingId.value) {
      if (schoolLogoInput.value) schoolLogoInput.value.value = "";
    }
  } catch {}
};

const loadSchools = () => schoolStore.loadSchools();
const loadAdminUsers = () => schoolStore.loadAdminUsers();
const submitSchool = async () => {
  try {
    const response = await schoolStore.submitSchool();
    if (response && schoolLogoInput.value) {
      schoolLogoInput.value.value = "";
    }
  } catch {}
};
const editSchool = (item) => {
  schoolStore.editSchool(item);
  if (schoolLogoInput.value) {
    schoolLogoInput.value.value = "";
  }
};
const resetForm = () => {
  schoolStore.resetSchoolForm();
  if (schoolLogoInput.value) {
    schoolLogoInput.value.value = "";
  }
};
const resetAdminForm = () => schoolStore.resetAdminForm();
const resetAdminFilters = () => schoolStore.resetAdminFilters();

const submitAdminUser = async () => {
  try {
    const wasEditing = Boolean(adminEditingId.value);
    const response = await schoolStore.submitAdminUser();
    pushToast({
...
const editAdminUser = (item) => {
  schoolStore.editAdminUser(item);
};
...
const deleteAdminUser = async (item) => {
  ...
  try {
    const response = await schoolStore.deleteAdminUser(item);
    pushToast({
...
watch(removeSchoolLogo, (value) => {
  schoolStore.setRemoveSchoolLogo(value);
});
