    ...
    <main class="mx-auto mt-8 max-w-[1400px] space-y-6">
      <section class="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-900/5 dark:bg-slate-900 dark:ring-white/10">
        ...
        <p class="mt-2 max-w-3xl text-sm leading-6 text-slate-500 dark:text-slate-400">
          Reset data dilakukan per modul dan hanya berlaku untuk sekolah admin yang sedang login. Setiap aksi wajib dikonfirmasi dengan mengetik <span class="font-bold text-slate-900 dark:text-white">RESET</span>.
        </p>
      </section>
      ...
      <Transition enter-active-class="transition ease-out duration-300" enter-from-class="opacity-0 -translate-y-2"
        ...
        <div v-if="message"
          class="flex items-center gap-3 rounded-2xl p-4 text-sm font-medium ring-1 ring-inset"
          :class="isError ? 'bg-red-50 text-red-700 ring-red-600/20' : 'bg-emerald-50 text-emerald-700 ring-emerald-600/20'">
          <svg v-if="isError" class="h-5 w-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
      ...
      <section class="rounded-3xl bg-white shadow-sm ring-1 ring-slate-900/5 dark:bg-slate-900 dark:ring-white/10">
        <div class="border-b border-slate-100 px-6 py-5 dark:border-slate-800">
          <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Pilih modul yang ingin dibersihkan tanpa harus menghapus semuanya sekaligus.</p>
            </div>
        ...
        <div class="p-6">
          <div v-if="isLoading" class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            <div v-for="item in 6" :key="item" class="h-48 animate-pulse rounded-3xl bg-slate-100 dark:bg-slate-800"></div>
          </div>
            ...
            <article v-for="item in settingsItems" :key="item.key"
              ...
              <div class="flex items-start justify-between gap-4">
                <div>
                  ...
                  <h3 class="mt-2 text-lg font-black tracking-tight text-slate-900 dark:text-white">{{ item.label }}</h3>
                </div>
                <div class="rounded-2xl bg-white px-3 py-2 text-right ring-1 ring-slate-200 dark:bg-slate-900 dark:ring-slate-700">
